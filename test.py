...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path',     default="./best_model/best_model.zip", help='Train from scratch')
    ...
    if args.mode == "eval":
        ...
        for i in range(50):
            ...
            for j in range(args.now):
                ...
                truncations[-1] = 1
                infos = {}
                episodes.append(minari.data_collector.EpisodeBuffer(j+i*args.now,
                                                observations=obss[j],
                                                actions=actions[j],
                                                rewards=rewards[j],
                                                ...
                                                infos = infos))
            print(f"Episode {i*args.now} collected with {obss[j].shape[0]} timesteps")
        dataset = minari.create_dataset_from_buffers(f'standard_{args.env}',
                                                     buffer = episodes,
