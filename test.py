...
if __name__ == "__main__":
    ...
    parser.add_argument('--gpu',           default=3, type=int, help='Random seed')
    ...
    if args.mode == "eval":
        ...
        episodes = []
        dynamics_models_list = glob.glob("./best_model/trained_tasks_RandomWalker2d-v0_*.npy")
        task_index = 0
        for dynamics_model_path in dynamics_models_list:
            for task in np.load(dynamics_model_path):

                if task_index == 0:
                    model = SAC.load(
                        f"./best_model/best_model_{args.env}_{default_task[:6]}.zip",
                        env=par_env,
                    ...
                    model = SAC.load(f"./best_model/best_model_{args.env}_{task[:6]}.zip", env=par_env, device=f"cuda:{args.gpu}" if torch.cuda.is_available() else "cpu")
                    print(f"Collecting data on task: {task}")
                    task_list = np.vstack([task_list,task[np.newaxis,:]])
                    par_env.set_task(np.tile(task, (args.now,1)))
            ...
            if task_index >= task_num:
                break
        dataset = minari.create_dataset_from_buffers(f'taggedmix{task_num}dynamics_{args.env}',
                                                     buffer = episodes,
